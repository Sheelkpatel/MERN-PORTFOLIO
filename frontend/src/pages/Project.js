  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        alert('Failed to fetch published projects');
      }
    };
    fetchProjects();
  }, []);
    ...
    <section id="project" className="py-5" style={{ backgroundColor: "#121212" }}>
      <div className="container">
        ...
        <div className="row gx-lg-5 gy-4 justify-content-center">
          {projects.length === 0 ? (
            <p className="text-center text-light">No published projects to show.</p>
          ) : (
            projects.map((project, index) => {
              const carouselId = `carousel-${project._id}`;

              return (
                <div key={project._id} className="col-12 col-md-6 col-lg-5 d-flex">
                  <motion.div
                    ...
                    >
                      ...
                      <div id={carouselId} className="carousel slide" data-bs-ride="carousel">
                        <div className="carousel-inner">
                          {(project.images?.length ? project.images : ['https://via.placeholder.com/400x250']).map((img, idx) => (
